...
llm_cpp = load(name="llm_extension_cpp", sources=["llm_extension.cpp"], verbose=True)
...
class CustomLinear(nn.Linear):
    ...
    def forward(self, x):
        if self.bias is not None:
            return llm_cpp.testGEMM(x, self.weight.transpose(0, 1).contiguous(), x.shape[0], self.weight.shape[0], x.shape[1], self.bias.unsqueeze(0).expand(x.shape[0], -1))
        else :
            return llm_cpp.testGEMM(x, self.weight.transpose(0, 1).contiguous(), x.shape[0], self.weight.shape[0], x.shape[1], torch.zeros(x.shape[0], self.weight.shape[0],dtype=torch.float16))
...
class TelechatAttention(nn.Module):
    def __init__(self, config: TelechatConfig, layer_idx):
        ...
        self.query = CustomLinear(self.hidden_size, self.hidden_size, bias=False)
        self.key_value = CustomLinear(self.hidden_size, kv_projection_size * 2, bias=False)
        self.dense = CustomLinear(self.hidden_size, self.hidden_size)
    ...
    def forward(
        ...
        # Step 1: Transpose the hidden states
        step_start_time = time.time()
        hidden_states = hidden_states.transpose(1, 0)
        step_time = time.time() - step_start_time
        print(f"Step 1 (Transpose hidden_states) time: {step_time:.6f} seconds")

        # Step 2: Query layer computation
        step_start_time = time.time()
        query_layer = self.query(hidden_states.transpose(0, 1).view(-1, hidden_states.size(-1))).view(hidden_states.size(0),hidden_states.size(1),-1)
        new_tensor_shape = query_layer.size()[:-1] + \
                           (self.num_heads,
                            self.head_dim)
        query_layer = query_layer.view(*new_tensor_shape)
        step_time = time.time() - step_start_time
        print(f"Step 2 (Query layer computation) time: {step_time:.6f} seconds")

        # Step 3: Key-Value layer computation
        step_start_time = time.time()
        mixed_kv_layer = self.key_value(hidden_states.transpose(0, 1).view(-1,hidden_states.size(-1))).view(hidden_states.size(0), hidden_states.size(1), -1)  # [seqlen, bsz, hidden_size] => [seqlen, bsz, head*head_dim*2]
        ...
        step_time = time.time() - step_start_time
        print(f"Step 3 (Key-Value layer computation) time: {step_time:.6f} seconds")
        ...
        step_time = time.time() - step_start_time
        print(f"Step 4 (Attention computation) time: {step_time:.6f} seconds")
        ...
        step_time = time.time() - step_start_time
        print(f"Step 5 (Attention score computation) time: {step_time:.6f} seconds")

        # Step 6: Dense layer
        step_start_time = time.time()
        output_tensor = self.dense(context_layer.view(-1, context_layer.size(-1))).view(bz, s_query, -1) # [bsz, seqlen, hidden_size]
        step_time = time.time() - step_start_time
        print(f"Step 6 (Dense layer) time: {step_time:.6f} seconds")

        # Step 6: Dense layer
        step_start_time = time.time()
        output_tensor = dropout_add(output_tensor, residual, self.hidden_dropout, self.training)
        step_time = time.time() - step_start_time
        print(f"Step 7 (Residual connection and dropout) time: {step_time:.6f} seconds")
        ...
        step_time = time.time() - step_start_time
        print(f"Final output preparation time: {step_time:.6f} seconds")
...
class TelechatMLP(nn.Module):
    def __init__(self, config: TelechatConfig):
        ...
        self.gate_proj = CustomLinear(self.hidden_size, config.ffn_hidden_size, bias=False)
        self.up_proj = CustomLinear(self.hidden_size, config.ffn_hidden_size, bias=False)
        self.down_proj = CustomLinear(config.ffn_hidden_size, self.hidden_size, bias=True)
        self.hidden_dropout = config.hidden_dropout

    def forward(self, hidden_states: torch.Tensor, residual: torch.Tensor) -> torch.Tensor: # [bsz, seqlen, hidden_size]
        bsz, seqlen, ffn_hidden_size = hidden_states.size()
        up_proj_res = self.up_proj(hidden_states.view(-1, hidden_states.size(-1))) # [bsz*seqlen, ffn_hidden_size]
        gate_proj_res = self.gate_proj(hidden_states.view(-1, hidden_states.size(-1))) # [bsz*seqlen, ffn_hidden_size]
        intermediate_output = self.down_proj(F.silu(gate_proj_res) * up_proj_res).view(bsz, seqlen, self.hidden_size) # [bsz, seqlen, hidden_size]
...
class TelechatBlock(nn.Module):
    ...
    def forward(
        ...
        print(f"input_layernorm time: {time.time() - start_time:.6f}s")
        ...
        print(f"self_attention time: {time.time() - start_time:.6f}s")
        start_time = time.time()

        attention_output = attn_outputs[0]
        outputs = attn_outputs[1:]
        layernorm_output = self.post_attention_layernorm(attention_output)
        print(f"post_attention_layernorm time: {time.time() - start_time:.6f}s")
        start_time = time.time()
        ...
        print(f"mlp time: {time.time() - start_time:.6f}s")
...
class TelechatForCausalLM(TelechatPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        decoded_tokens = [tokenizer.decode(id) for id in input_list]
        print(f"out put token: {decoded_tokens}")
        model_inputs.update(
            {
    ...
    def forward(
        ...
        lm_logits = self.lm_head(hidden_states.view(bsz * seq_length, -1)).view(bsz, seq_length, -1)
